...
class BiometricViewModel : ViewModel() {
    private val _loginForm = MutableLiveData<BiometricFormState>()
    val loginWithPasswordFormState: LiveData<BiometricFormState> = _loginForm

    private val _loginResult = MutableLiveData<BiometricResult>()
    val loginResult: LiveData<BiometricResult> = _loginResult
    ...
    fun login(username: String, password: String) {
        if (isUserNameValid(username) && isPasswordValid(password)) {
            ...
            _loginResult.value = BiometricResult(true)
        } else {
            _loginResult.value = BiometricResult(false)
        }
    }
}